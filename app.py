...
def offerWord(pangramString):
    ...
    if len(pangramString) < 7:
        ...
        messages=[
            ...
            {"role": "user", "content": "Out of these words, which is the most popular? " + pangramString + "Just respond with the more popular word. Your response should only be one word, and have no special characters in it. it should be all lowercase. If there are no words, return 'There is no pangram'. "} #pangramString ensured to be a string of pangrams separated by commas
        ]
        )
